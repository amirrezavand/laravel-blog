...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $courses=Course::leftJoin('users','users.id','=','courses.user_id')->where('course.lu_content_status','pulish')->select(DB::raw('courses.*'),DB::raw('users.name as teacher_name'),DB::raw('users.avatar as teacher_avatar'))->orderby('id','asc')->limit(6)->get();
